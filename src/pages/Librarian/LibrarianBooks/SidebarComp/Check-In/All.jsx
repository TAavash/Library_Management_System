...
const All = () => {
  ...
  const selectedBook = checkIn.find((item) => item.id === bookId);
      ...
      <div className="flex-col h-[80%] bg-white p-[30px] gap-[30px] rounded-br-2xl overflow-y-auto scroll-smooth scrollbar-thin">
        <div className="grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-[30px] my-2">
          {checkIn.map((check) => (
            <div
              key={check.id}
              className="group relative bg-white cursor-pointer rounded-lg shadow-md w-56 overflow-hidden transition-transform transform hover:scale-105 hover:shadow-xl"
